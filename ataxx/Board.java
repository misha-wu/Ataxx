...
class Board {
    ...
    void clear() {
        ...
        unrecordedSet('a', '1', PieceColor.BLUE);
        unrecordedSet('g', '1', PieceColor.RED);

        unrecordedSet('a', '7', PieceColor.RED);
        unrecordedSet('g', '7', PieceColor.BLUE);

        _numPieces[PieceColor.RED.ordinal()] = 2;
        _numPieces[PieceColor.BLUE.ordinal()] = 2;
        _winner = null;
        _undoSquares = new Stack<>();
        _undoPieces = new Stack<>();
        _allMoves = new ArrayList<>();
    ...
    boolean legalMove(Move move) {
        if (move != null) {
            ...
            PieceColor intended = get(move.col0(), move.row0());
            if (getWinner() != null) {
                return false;
            } else if (intended != whoseMove()) {
                return false;
            } else if (canMove(intended)
    ...
    ArrayList<Move> moveList(PieceColor pl) {
        ArrayList moves = new ArrayList<Move>();
        for (char c = 'a'; c <= 'g'; c++) {
            for (char r = '1'; r <= '7'; r++) {
                if (get(c, r) == pl) {
                    moveListHelper(c, r, moves);
                }
            }
        }
        if (moves.size() == 0) {
    ...
    void moveListHelper(char col, char row, ArrayList s) {
        for (char r = (char) (row - 2); r <= row + 2; r++) {
            for (char c = (char) (col - 2); c <= col + 2; c++) {
                if (get(c, r) == EMPTY) {
                    s.add(Move.move(col, row, c, r));
                }
            }
        }
    }

    /** returns true iff row, col can move to another spot on board.
     * ignoring if rc is right color, etc
     * @param col
     * @param row
     * @return if there is possible move
     */
    boolean innerMove(char col, char row) {
        for (char r = (char) (row - 2); r <= row + 2; r++) {
            for (char c = (char) (col - 2); c <= col + 2; c++) {
                if (get(c, r) == EMPTY) {
                    return true;
    ...
    void makeMove(Move move) {
        ...
        char newc = move.col1(); char newr = move.row1();

        if (move.isExtend()) {
            setSquare(newc, newr, whoseMove(), opponent);
            _numJumps = 0;

        } else if (move.isJump()) {
            set(move.col0(), move.row0(), EMPTY);
            _numPieces[whoseMove().ordinal()] -= 1;
            setSquare(newc, newr, whoseMove(), opponent);
            _numJumps += 1;
        ...
        if ((!canMove(opponent) && !canMove(whoseMove()))
                || numJumps() == JUMP_LIMIT
                || numPieces(RED) == 0
                || numPieces(BLUE) == 0) {
    ...
    void setSquare(char c, char r, PieceColor who, PieceColor opponent) {

        set(c, r, who);
        _numPieces[who.ordinal()] += 1;

        for (char i = (char) (r - 1); i <= r + 1; i++) {
            for (char j = (char) (c - 1); j <= c + 1; j++) {
                if (get(j, i) == opponent) {
                    set(j, i, who);
                    _numPieces[who.ordinal()] += 1;
    ...
    void undo() {
        while (_undoSquares.peek() != null) {
            int revertLoc = _undoSquares.pop();
            PieceColor oldColor = _undoPieces.pop();
            PieceColor currColor = get(revertLoc);

            unrecordedSet(revertLoc, oldColor);
            if (currColor == RED || currColor == BLUE) {
                _numPieces[currColor.ordinal()] -= 1;
            }
            _numPieces[oldColor.ordinal()] += 1;
        }
    ...
    boolean legalBlock(char c, char r) {
        ...
        char[][] corners = cornerArr(c, r);
        for (char[] i : corners) {
            if (get(i[0], i[1]) != EMPTY) {
                return false;
            }
    ...
    char[][] cornerArr(char c, char r) {
        char cm = (char) ('a' + Math.ceil(SIDE / 2));
        char rm = (char) ('1' + Math.ceil(SIDE / 2));

        int cdist = Math.abs(cm - c);
        int rdist = Math.abs(rm - r);

        char[][] corners = {{(char) (cm - cdist),
                (char) (rm + rdist)},
            {(char) (cm - cdist), (char) (rm - rdist)},
            {(char) (cm + cdist), (char) (rm + rdist)},
            {(char) (cm + cdist), (char) (rm - rdist)}};

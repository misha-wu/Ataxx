...
class Move {
    ...
    private Move(int col0, int row0, int col1, int row1) {
        ...
        if (Math.abs(_col1 - _col0) <= 1 && Math.abs(_row0 - _row1) <= 1) {
            _isJump = false;
            _isExtend = true;
        } else {
            _isJump = true;
    ...
    public String toString() {
        ...
        String from = _col0 + Character.toString(_row0);
        String to = _col1 + Character.toString(_row1);
    ...
    private final boolean _isExtend;
    /** Move characteristic, true if Jump.**/
    private final boolean _isJump;

    /** From and two squares, or 0s if a pass. */
    private final char _col0;
    /** the old row. */
    private final char _row0;
    /** the new col. */
    private final char _col1;
    /** the new row. */
    private final char _row1;
